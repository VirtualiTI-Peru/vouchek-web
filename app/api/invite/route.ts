...
export async function POST(req: NextRequest) {
  try {
    ...
    if (invitationError) {
      return NextResponse.json({ error: mapSupabaseError(invitationError.message) || "Error al crear la invitación" }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: error?.message || ApiErrors.SEND_INVITATION }, { status: 500 });
  }
}
